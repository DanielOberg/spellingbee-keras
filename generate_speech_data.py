...
def generate_mfcc(voice, word, rate, path):
	filename = path+"/wav/{0}_{1}_{2}.wav".format(word, voice, rate)
	cmd = "say '{0}' -v{1} -r{2} --data-format=LEI16@44100 -o '{3}'".format(word, voice, rate, filename)
	os.system(cmd)  # ogg aiff m4a or caff
        cmd = "ffmpeg -y -i {0} -f s16le -acodec pcm_s16le {1}".format(filename, path+"/raw/{0}_{1}_{2}.pcm".format(word, voice, rate))
	os.system(cmd)
...
def generate(words, path):
	...
	if not os.path.exists(path + "/raw/"): os.mkdir(path + "/raw/")        
	...
	for word in words:
		...
		#rate=120
		for rate in range(80,360,20):
		    for voice in good_voices:
		        generate_mfcc(voice, word, rate, path)
		        out.write("{0}_{1}_{2}.pcm,{3}\n".format(word, voice, rate,word))
